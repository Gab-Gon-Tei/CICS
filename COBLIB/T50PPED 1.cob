      ...
      *
       77  WS-QUEUE-NAME                   PIC X(08)
                                                   VALUE 'TS08PPED'.
       77  WS-TRIM                         PIC 9(02).
       77  WS-ALL-SPACES                   PIC X(15) VALUE ZEROS.
       77  WS-PED-AUX                      PIC X(15).
       77  WS-VENDA-AUX                    PIC X(15).
       01  LD-LINDET.
           05 FILLER                       PIC X(04)
                                                   VALUE SPACES.
           05 LD-ID-PED                    PIC X(09).
      ...
      *TROCAR CPF HARDCODE PARA O DA DFH
           ...
           END-IF
           IF PEDPEDL > 0
              EXEC CICS BIF DEEDIT FIELD(PEDPEDI) END-EXEC
              MOVE PEDPEDI                 TO WS-FILTRO-P
      *       MOVE 'ABC'                   TO WS-FILTRO-P
           END-IF
      *       MOVE SPACES                  TO WS-FILTRO-P
      ...
      *    END-IF
           ...
           IF DFPEDL NOT < 1 AND
              ...
              IF RESP = 1
               ...
               END-IF
           ELSE
              MOVE '28'                     TO WS-FINAL-DIA
              MOVE '12'                     TO WS-FINAL-MES
              MOVE '2099'                   TO WS-FINAL-ANO
              MOVE WS-DATA-FINAL            TO WS-DATA-FINAL-F
           END-IF
      ...
      *
           ...
           .
       999-MANDA-TELA.
           MOVE EIBTRMID                    TO TERMPEDO
           MOVE EIBTRNID                    TO TRANSPEDO
           MOVE EIBTASKN                    TO TASKPEDO
           MOVE WS-FASE                     TO FASEPEDO
      *
           EXEC CICS LINK
              PROGRAM('AUXCICS1')
      ...
      *
       ...
       999-TRATA-FASE2.
           ...
           MOVE WS-PEDPAG                  TO PAGPEDO
           MOVE WS-PEDPAGD                 TO DEPEDO
           PERFORM 999-MANDA-TELA
           PERFORM 999-CHAMA-FASE2
           .
       999-MAPFAIL.
           MOVE 'ERRO MAPA T50PPED'       TO WS-MSG-ERRO
           PERFORM 999-ENCERRA-TRANSACAO
           .
       999-ERRO-GENERICO.
           MOVE WS-FILTRO               TO WS-MSG-ERRO
            MOVE SQLCODE                TO WS-MSG-ERRO(1:5).
               MOVE ' <- SQLCODE'       TO WS-MSG-ERRO(6:16).
               PERFORM 999-ENCERRA-TRANSACAO
           PERFORM 999-ENCERRA-TRANSACAO
           .
       999-NOTFND.
      ...
      -         'MADO'                     TO MSGPEDO
       ...
       999-CARREGA-DETALHES.
           ...
           PERFORM WITH TEST AFTER UNTIL WS-INDEX = 10
              ...
              IF WS-RESP = DFHRESP(NORMAL)
                 ADD 1 TO WS-INDEX
                 MOVE LD-LINDET            TO LINPEDO(WS-INDEX)
              ELSE
                 IF WS-RESP = DFHRESP(ITEMERR)
                    PERFORM WITH TEST AFTER UNTIL WS-INDEX = 10
                       ADD 1 TO WS-INDEX
                       MOVE SPACES         TO LINPEDO(WS-INDEX)
                    END-PERFORM
                 ELSE
                    PERFORM 999-ERRO-GENERICO
                 END-IF
       ...
       999-VALIDA-POSICAO.
           ...
           MOVE LINPEDI(WS-INDEX)       TO LD-LINDET
           IF LD-ITEM  EQUAL SPACES
      ...
      *                                    INTO PRLMSGO
           .
       999-PROTECAO-FASE2.
           MOVE DFHUNDLN                   TO PEDPEDH
           MOVE DFHUNDLN                   TO DIPEDH
           MOVE DFHUNDLN                   TO MEPEDH
           MOVE DFHUNDLN                   TO ANPEDH
           MOVE DFHUNDLN                   TO DFPEDH
           MOVE DFHUNDLN                   TO MFPEDH
           MOVE DFHUNDLN                   TO AFPEDH
           MOVE DFHUNDLN                   TO VTPEDH
      *
