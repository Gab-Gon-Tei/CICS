      ...
      *    .
        210-PF1.
           IF EIBCPOSN <= 400 OR EIBCPOSN >= 1360
              MOVE 'POSICAO INVALIDA DO CURSOR'
                                           TO T3MSGO
             PERFORM 999-TRATA-FASE2
           END-IF

           COMPUTE WS-INDEX = EIBCPOSN / 80 + 1 - 5
           ...
           MOVE LD-ID                     TO WS-ID
           MOVE WS-ID                     TO DCLCAR-ID-PRODUTO
           MOVE WS-ID-PEDIDO              TO DCLCAR-ID-PEDIDO
           MOVE LD-VALOR                TO DCLCAR-VALOR-UNITARIO-PEDIDO

           EXEC SQL
             DELETE FROM CARRINHO 
               WHERE ID_PEDIDO = :DCLCAR-ID-PEDIDO
                 AND ID_PRODUTO = :DCLCAR-ID-PRODUTO
           END-EXEC
           ...
           EVALUATE SQLCODE
               WHEN 0
                    ...
                    ' REMOVIDO DO CARRINHO' DELIMITED BY SIZE
                                           INTO T3MSGO
                    SUBTRACT 1 FROM WS-CONT-CARR
                    PERFORM 999-UPDATE-PEDIDO
                    PERFORM 999-CHAMA-CURSOR
               WHEN +100
      ...
      *----------------------------------------------------------------*
        999-UPDATE-PEDIDO.
           EXEC SQL
               UPDATE PEDIDOS
               SET VALOR_TOTAL = 
               VALOR_TOTAL - :DCLCAR-VALOR-UNITARIO-PEDIDO
               WHERE ID_PEDIDO = :DCLCAR-ID-PEDIDO
           END-EXEC
             EVALUATE SQLCODE
                WHEN 0
                     CONTINUE
                WHEN +100
                     MOVE 'PEDIDO NAO ENCONTRADO'
                                              TO T3MSGO
                WHEN OTHER
                     MOVE 'ERRO AO ATUALIZAR PEDIDO'
                                              TO T3MSGO
                     PERFORM 999-ERROR
             END-EVALUATE
           .
